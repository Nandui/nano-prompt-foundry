...
const roleInstructions = {
  face:
    "Analyze only the face/head identity reference. Describe facial structure, face shape, jaw/chin, cheekbones, eyes, eyebrows, nose, lips, expression, hair color/style/hairline, skin texture, makeup, jewelry or eyewear if visible, and identity-critical anchors. Do not describe the room, selfie action, camera, clothing, body, or background. Do not infer private identity. Return fields: description, parts.face_shape, parts.eyes_brows, parts.nose, parts.lips, parts.hair, parts.expression, parts.skin_texture, parts.makeup, parts.jewelry_eyewear, parts.identity_anchors, parts.do_not_transfer.",
  body:
...
export function analysisPrompt(role, metadata) {
  return [
    ...
    role === "face"
      ? "For face references, never mention bedroom, selfie, phone, camera angle, outfit, body pose, or background. The description should be face/head identity only."
      : "",
    ...
    "Keep the JSON complete but concise. Description can be 70-95 words. Each parts value can be 15-35 words.",
...
function formatAnalysisText(structured, fallbackText) {
  if (!structured?.parts || !Object.keys(structured.parts).length) {
    return structured?.description || fallbackText;
  }

  const partLines = Object.entries(structured.parts)
    .filter(([, value]) => typeof value === "string" && value.trim())
    .map(([key, value]) => `${key}: ${value.trim()}`);

  return [structured.description, ...partLines].filter(Boolean).join("\n");
}
...
function buildAnalysisResult(payload, providerName) {
  ...
  return {
    analysis: formatAnalysisText(structured, text),
