...
function analysisPrompt(role, metadata) {
  return [
...
async function analyzeImage({ role, imageDataUrl, metadata }) {
...
async function analyzeWithOpenAI({ role, imageDataUrl, metadata }) {
  if (!process.env.OPENAI_API_KEY) {
    return {
      setup_required: true,
      error:
        "OPENAI_API_KEY is not set. Start the local server with your OpenAI API key to use vision analysis."
    };
  }
      ...
      input: [
        {
          ...
          content: [
            {
              type: "input_text",
              text: analysisPrompt(role, metadata)
            },
            {
              type: "input_image",
  ...
  if (!apiResponse.ok) {
    return {
      error: payload.error?.message || "OpenAI vision analysis failed."
    };
  }

  return {
    analysis:
      parseStructuredAnalysis(payload.output_text)?.description || payload.output_text || "No analysis text returned.",
    structured_analysis: parseStructuredAnalysis(payload.output_text),
    model: visionModel,
    provider: "openai"
  };
}
...
async function analyzeWithApiyi({ role, imageDataUrl, metadata }) {
  if (!process.env.APIYI_API_KEY) {
    return {
      setup_required: true,
      error:
        "APIYI_API_KEY is not set. Start the local server with your APIYi key to use Gemini vision analysis."
    };
  }
  ...
  if (!apiResponse.ok) {
    return {
      error: payload.error?.message || payload.message || "APIYi Gemini vision analysis failed."
    };
  }

  return {
    analysis:
      parseStructuredAnalysis(payload.choices?.[0]?.message?.content)?.description ||
      payload.choices?.[0]?.message?.content ||
      "No analysis text returned.",
    structured_analysis: parseStructuredAnalysis(payload.choices?.[0]?.message?.content),
    model: visionModel,
    provider: "apiyi"
  };
}
